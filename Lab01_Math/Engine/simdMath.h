...
public:
    ...
    friend SIMDVector3 operator*(const SIMDVector3& vec, float scalar)
    {
        ...
        return SIMDVector3(_mm_mul_ps(vec.mVec, scalarVec));
    }
    ...
    {
        __m128 scalarVec = _mm_set1_ps(scalar);
        return SIMDVector3(_mm_div_ps(vec.mVec, scalarVec));
    }
    ...
    {
        __m128 scalarVec = _mm_set1_ps(scalar);
        mVec = _mm_div_ps(mVec, scalarVec);
        return *this;
    }
    ...
    {
        __m128 squared = _mm_mul_ps(mVec, mVec);
        __m128 sum = _mm_hadd_ps(squared, squared); 
        sum = _mm_hadd_ps(sum, sum);              
        return SIMDVector3(_mm_set1_ps(_mm_cvtss_f32(sum)));
    }
    ...
    SIMDVector3 Length_v() const
    {
        ...
        __m128 sum = _mm_hadd_ps(squared, squared);
        sum = _mm_hadd_ps(sum, sum);              
        return SIMDVector3(_mm_sqrt_ps(_mm_set1_ps(_mm_cvtss_f32(sum))));
    }
    ...
    friend SIMDVector3 Cross(const SIMDVector3& a, const SIMDVector3& b)
    {
        ...
        return SIMDVector3(_mm_sub_ps(lhs, rhs));
    }
    ...
    friend SIMDVector3 Lerp(const SIMDVector3& a, const SIMDVector3& b, float f)
    {
        ...
        __m128 delta = _mm_sub_ps(b.mVec, a.mVec);
        __m128 scaledDelta = _mm_mul_ps(delta, fVec);
        return SIMDVector3(_mm_add_ps(a.mVec, scaledDelta));
    }
...
public:
    ...
    {
        __m128 scalarVec = _mm_set1_ps(scalar);
        return SIMDVector4(_mm_mul_ps(vec.mVec, scalarVec));
    }
    ...
    {
        __m128 scalarVec = _mm_set1_ps(scalar);
        return SIMDVector4(_mm_div_ps(vec.mVec, scalarVec));
    }
    ...
    SIMDVector4 LengthSq_v() const
    {
        ...
        __m128 sum = _mm_hadd_ps(squared, squared);
        sum = _mm_hadd_ps(sum, sum);
        return SIMDVector4(_mm_set1_ps(_mm_cvtss_f32(sum)));
    }
    ...
    SIMDVector4 Length_v() const
    {
        ...
        __m128 sum = _mm_hadd_ps(squared, squared);
        sum = _mm_hadd_ps(sum, sum);
        return SIMDVector4(_mm_sqrt_ps(_mm_set1_ps(_mm_cvtss_f32(sum))));
    }
    ...
    void Normalize()
    {
        __m128 temp = _mm_dp_ps(mVec, mVec, 0xFF);
        temp = _mm_rsqrt_ps(temp);
        mVec = _mm_mul_ps(mVec, temp);
    }
    ...
    {
        __m128 temp = _mm_dp_ps(vec.mVec, vec.mVec, 0xFF);
        temp = _mm_rsqrt_ps(temp);
        return SIMDVector4(_mm_mul_ps(vec.mVec, temp));
    }
    ...
    friend SIMDVector4 Cross(const SIMDVector4& a, const SIMDVector4& b)
    {
        ...
        tempB = _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLER(1, 2, 0, 3));
        __m128 rhs = _mm_mul_ps(tempA, tempB);
        return SIMDVector4(_mm_sub_ps(lhs, rhs));
    }
    ...
    friend SIMDVector4 Lerp(const SIMDVector4& a, const SIMDVector4& b, float f)
    {
        ...
        __m128 delta = _mm_sub_ps(b.mVec, a.mVec);
        __m128 scaledDelta = _mm_mul_ps(delta, fVec);
        return SIMDVector4(_mm_add_ps(a.mVec, scaledDelta));
    }
