...
FollowCam::FollowCam(RenderObj* pObj, Game* pGame)
    ...
    , mGame(pGame)
{
	Matrix4 mat = mRenderObj->mPerObjectConst.c_modelToWorld;
...
void FollowCam::Update(float deltaTime)
{
    ...
    Matrix4 mat = Matrix4::CreateTranslation(mRenderObj->mPerObjectConst.c_modelToWorld.GetTranslation() + mOffset);
    mat.Invert();
