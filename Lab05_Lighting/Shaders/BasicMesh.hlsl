...
float4 PS(VOut pIn) : SV_TARGET
{
    ...
    for (int i = 0; i < MAX_POINT_LIGHTS; ++i)
    {
        ...
        if (light.isEnabled)
        {
            ...
            float attenuation = 1.0f - smoothstep(light.outerRadius, light.innerRadius, distance);
    ...
    float4 color = texColor * pIn.color;
    float3 finalColor = color.rgb * accumulatedLight;
